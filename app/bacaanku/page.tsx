...
export default function BacaankuPage() {
  ...
  const renderList = (data: BookmarkData[], type: string, icon: ReactNode) => {
    ...
    return (
      <div className={type === 'history' ? 'divide-y divide-white/10' : 'grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4'}>
        {sorted.length === 0 ? (
          <div className="text-gray-400 col-span-full">Belum ada data.</div>
        ) : sorted.map((item, idx) => (
          <div key={item.idKomik + '-' + item.chapterId} className={type === 'history' ? 'flex items-center gap-3 py-3 px-2 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 transition' : 'flex items-center gap-3 p-4 bg-white/5 rounded-lg hover:bg-white/10 transition'}>
            ...
            <div className="flex-1 min-w-0">
              <Link href={`/${item.idKomik}/chapter/${item.chapterId}${item.page ? `?page=${item.page}` : ''}`} className="font-semibold text-white text-sm truncate block">
                {item.title}
              </Link>
              <div className="text-xs text-white/60 truncate flex items-center gap-2">
                {item.chapterTitle}{item.page ? ` • Halaman ${item.page}` : ''}
                {type === 'history' && (
                  <span className={`ml-2 px-2 py-0.5 text-[10px] rounded-full font-bold ${item.isPaid ? 'bg-yellow-400 text-yellow-900' : 'bg-green-500 text-white'}`}>{item.isPaid ? 'Berbayar' : 'Free'}</span>
                )}
              </div>
              {type === 'history' && item.lastRead && (
                <div className="text-[10px] text-white/40 mt-1">Terakhir dibaca: {new Date(item.lastRead).toLocaleString('id-ID')}</div>
              )}
            </div>
            <button
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6">Bacaanku</h1>
      <div className="flex gap-2 mb-6">
        <button onClick={() => setTab('bookmark')} className={`px-4 py-2 rounded-full font-semibold ${tab==='bookmark' ? 'bg-purple-600 text-white' : 'bg-white/10 text-white'}`}><Bookmark className="inline w-4 h-4 mr-1" />Bookmark</button>
        <button onClick={() => setTab('like')} className={`px-4 py-2 rounded-full font-semibold ${tab==='like' ? 'bg-purple-600 text-white' : 'bg-white/10 text-white'}`}><ThumbsUp className="inline w-4 h-4 mr-1" />Like</button>
        <button onClick={() => setTab('history')} className={`px-4 py-2 rounded-full font-semibold ${tab==='history' ? 'bg-purple-600 text-white' : 'bg-white/10 text-white'}`}><BookOpen className="inline w-4 h-4 mr-1" />Riwayat</button>
      </div>
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-white rounded-xl p-6 shadow-xl max-w-xs w-full text-center relative">
            ...
            <div className="mb-4 text-black font-semibold">Yakin ingin menghapus?</div>
            <div className="flex gap-2 justify-center">
              <button onClick={() => setShowConfirm(null)} className="px-4 py-2 rounded bg-gray-200 hover:bg-gray-300 text-black font-semibold">Batal</button>
              <button onClick={() => handleDelete(showConfirm.type, showConfirm.idx)} className="px-4 py-2 rounded bg-red-600 hover:bg-red-700 text-white font-semibold">Hapus</button>
            </div>
          </div>
        </div>
      )}
